...
function Countdown({ date }: { date: string }) {
  ...
  const units = [{ v: d, l: 'Dias' }, { v: h, l: 'Horas' }, { v: m, l: 'Min' }, { v: s, l: 'Seg' }]
  return (
        ...
        <div key={l} style={{ display: 'flex', alignItems: 'flex-start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ background: '#0A0A0A', padding: '0.85rem 1rem', minWidth: 'clamp(4.5rem, 8vw, 7rem)', textAlign: 'center' }}>
              <span className="display" style={{ fontSize: 'clamp(2.5rem, 5vw, 4.5rem)', color: '#FF7B00', lineHeight: 1, fontVariantNumeric: 'tabular-nums', display: 'block' }}>
                {String(v).padStart(2, '0')}
              </span>
            </div>
            <span className="mono" style={{ fontSize: '0.78rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#999', marginTop: '0.5rem', fontWeight: 700 }}>{l}</span>
          </div>
          {i < units.length - 1 && (
...
function Carousel() {
  ...
  return (
    <div style={{ position: 'relative', overflow: 'hidden', background: '#000', width: '100%', aspectRatio: '16/8' }}
      ...
      <AnimatePresence custom={dir} initial={false}>
        <motion.div key={cur} custom={dir}
          ...
          <img src={slides[cur].src} alt={slides[cur].caption}
               style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.6) 0%, transparent 55%)' }} />
          <motion.p className="mono"
            initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
            style={{ position: 'absolute', bottom: '2rem', left: 'clamp(1.5rem, 5vw, 7rem)', fontSize: '0.78rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.65)', fontWeight: 700 }}>
            {slides[cur].caption}
          </motion.p>
        </motion.div>
      </AnimatePresence>

      <button onClick={() => goTo(cur - 1, -1)}
        style={{ position: 'absolute', top: '50%', transform: 'translateY(-50%)', left: 'clamp(1rem, 3vw, 3rem)', zIndex: 10, width: '3rem', height: '3rem', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.14)', backdropFilter: 'blur(10px)', color: '#fff', cursor: 'pointer', transition: 'background 0.2s' }}
        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.18)' }}
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.08)' }}>
        <ChevronLeft size={20} />
      </button>
      <button onClick={() => goTo(cur + 1, 1)}
        style={{ position: 'absolute', top: '50%', transform: 'translateY(-50%)', right: 'clamp(1rem, 3vw, 3rem)', zIndex: 10, width: '3rem', height: '3rem', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.14)', backdropFilter: 'blur(10px)', color: '#fff', cursor: 'pointer', transition: 'background 0.2s' }}
        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.18)' }}
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.08)' }}>
        <ChevronRight size={20} />
      </button>

      <div className="mono" style={{ position: 'absolute', top: '2rem', right: 'clamp(1.5rem, 5vw, 7rem)', zIndex: 10, fontSize: '0.78rem', letterSpacing: '0.14em', color: 'rgba(255,255,255,0.5)', fontWeight: 700 }}>
        {String(cur + 1).padStart(2, '0')} / {String(total).padStart(2, '0')}
      </div>
      <div style={{ position: 'absolute', bottom: '2rem', right: 'clamp(1.5rem, 5vw, 7rem)', zIndex: 10, display: 'flex', gap: '0.4rem' }}>
        {slides.map((_, i) => (
          <button key={i} onClick={() => goTo(i, i > cur ? 1 : -1)}
            style={{ height: '2px', border: 'none', cursor: 'pointer', padding: 0, width: i === cur ? '2.5rem' : '0.9rem', background: i === cur ? '#FF7B00' : 'rgba(255,255,255,0.25)', transition: 'all 0.35s' }} />
        ))}
      </div>
    </div>
  )
    ...
    <main style={{ background: '#fff' }}>
      ...
      <section ref={heroRef} style={{
        ...
        >
          <div style={{ paddingBottom: 'clamp(4rem, 8vh, 6rem)', maxWidth: '780px' }}>
            ...
            >
              <div style={{ width: '2rem', height: '2px', background: '#FF7B00', flexShrink: 0 }} />
              <span className="mono" style={{ fontSize: '0.78rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: '#FF7B00', fontWeight: 700 }}>
            ...
            >
              ...
              <Link to="/expedicoes" className="mono"
                style={{ fontSize: '0.78rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem', transition: 'color 0.2s', fontWeight: 700 }}
        ...
        <motion.div
          ...
          <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
            style={{ width: '1px', height: '3.5rem', background: 'linear-gradient(to bottom, transparent, rgba(255,255,255,0.25))' }} />
          <span className="mono" style={{ fontSize: '0.6rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.2)', writingMode: 'vertical-rl', transform: 'rotate(180deg)' }}>scroll</span>
        </motion.div>
      </section>
      ...
      <section style={{ background: '#fff', padding: 'clamp(7rem, 14vh, 12rem) 0', borderBottom: '1px solid #EBEBEB' }}>
        <div className="wrap">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(3rem, 6vw, 8rem)', alignItems: 'end' }}
            ...
            <Reveal delay={0.15}>
              <p style={{ color: '#555', fontSize: '1.1rem', fontWeight: 300, lineHeight: 1.85, marginBottom: '2.5rem' }}>
          ...
          <Reveal delay={0.2}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', marginTop: 'clamp(5rem, 9vh, 8rem)', border: '1px solid #EBEBEB' }}>
              {[
                ...
                <div key={n} style={{ padding: 'clamp(2rem, 4vw, 3.5rem)', borderRight: i < 2 ? '1px solid #EBEBEB' : 'none' }}>
                  <p className="display" style={{ fontSize: 'clamp(2.5rem, 5vw, 4.5rem)', color: '#0A0A0A', marginBottom: '0.5rem' }}>{n}</p>
                  <p style={{ color: '#0A0A0A', fontSize: '1rem', fontWeight: 600, marginBottom: '0.3rem' }}>{label}</p>
                  <p style={{ color: '#888', fontSize: '0.9rem', fontWeight: 300 }}>{sub}</p>
                </div>
              ))}
            </div>
          </Reveal>
        ...
        <div className="wrap" style={{ marginBottom: '2.5rem' }}>
          <Reveal style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1.5rem' }}>
            <div>
              ...
              <h2 className="display" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.8rem)', color: '#0A0A0A' }}>NOSSAS EXPEDIÇÕES</h2>
            </div>
            <Link to="/expedicoes" className="mono"
              style={{ fontSize: '0.78rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#888', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem', paddingBottom: '0.25rem', transition: 'color 0.2s', fontWeight: 700 }}
      ...
      <section style={{ background: '#F7F7F5', padding: 'clamp(6rem, 11vh, 10rem) 0', borderTop: '1px solid #EBEBEB' }}>
        <div className="wrap">
          <Reveal>
            ...
            <h2 className="display" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.8rem)', color: '#0A0A0A', marginBottom: 'clamp(3rem, 5vh, 4rem)' }}>
              A PRÓXIMA AVENTURA JÁ TEM DESTINO
            </h2>
          </Reveal>

          <Reveal delay={0.1}>
            <div style={{ background: '#fff', border: '1px solid #EBEBEB', padding: 'clamp(2rem, 4vw, 3.5rem)', marginBottom: '2rem', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '2.5rem' }}>
              <div>
                <p className="mono" style={{ fontSize: '0.78rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#777', marginBottom: '1.5rem', fontWeight: 700 }}>
                  A expedição começa em
                </p>
                <Countdown date="2026-08-21T08:00:00" />
              </div>
              <div style={{ textAlign: 'right' }}>
                <p className="mono" style={{ fontSize: '0.78rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#777', marginBottom: '0.5rem', fontWeight: 700 }}>Investimento</p>
                <p className="display" style={{ fontSize: 'clamp(2.5rem, 4vw, 3.5rem)', color: '#0A0A0A' }}>R$ 3.800</p>
                <p className="mono" style={{ fontSize: '0.75rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#888', marginTop: '0.3rem', fontWeight: 700 }}>por pessoa · 7 vagas restantes</p>
              </div>
            </div>
          </Reveal>
          ...
          <Reveal delay={0.15}>
            <Link to="/expedicoes" style={{ textDecoration: 'none', display: 'block' }}>
              <div style={{ position: 'relative', overflow: 'hidden', background: '#000', border: '1px solid #EBEBEB', cursor: 'pointer' }}
                ...
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.55) 45%, rgba(0,0,0,0.1) 100%)' }} />
                <div style={{ position: 'absolute', inset: 0, padding: 'clamp(2rem, 4vw, 4.5rem)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.4rem 1rem', background: '#FF7B00', marginBottom: '1.75rem', width: 'fit-content' }}>
                    ...
                    <span className="mono" style={{ fontSize: '0.75rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#fff', fontWeight: 700 }}>Vagas abertas</span>
                  </div>
                  <h3 className="display" style={{ fontSize: 'clamp(3rem, 7vw, 6.5rem)', color: '#fff', lineHeight: 0.88, marginBottom: '0.3em' }}>CAMPOS DO JORDÃO</h3>
                  <p className="mono" style={{ fontSize: '0.82rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.55)', marginBottom: '2rem', fontWeight: 700 }}>
                    São Paulo · Serra da Mantiqueira · 21–28 Ago 2026
                  </p>
                  <div style={{ display: 'flex', gap: '2.5rem', flexWrap: 'wrap', marginBottom: '2.5rem' }}>
                    {[['8 dias', '7 noites'], ['7 vagas', 'restantes'], ['R$ 3.800', 'por pessoa']].map(([v, l]) => (
                      <div key={v}>
                        ...
                        <p className="mono" style={{ fontSize: '0.75rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', fontWeight: 700 }}>{l}</p>
                      </div>
                    ))}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'rgba(255,255,255,0.5)' }}>
                    <span className="mono" style={{ fontSize: '0.78rem', letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 700 }}>Ver roteiro completo</span>
                    <ArrowRight size={14} />
                  </div>
                </div>
              </div>
      ...
      <section style={{ background: '#0A0A0A', position: 'relative', overflow: 'hidden', padding: 'clamp(7rem, 14vh, 12rem) 0', textAlign: 'center', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
        ...
        <div className="wrap" style={{ position: 'relative', zIndex: 2 }}>
          ...
          <Reveal delay={0.1}>
            <h2 className="display" style={{ fontSize: 'clamp(3rem, 8vw, 7rem)', color: '#fff', lineHeight: 0.9, marginBottom: '0.12em' }}>PRONTO PARA</h2>
            <h2 className="display" style={{ fontSize: 'clamp(3rem, 8vw, 7rem)', lineHeight: 0.9, marginBottom: 'clamp(2rem, 4vh, 3rem)', WebkitTextStroke: '2px #FF7B00', color: 'transparent' }}>EMBARCAR?</h2>
          </Reveal>
          <Reveal delay={0.2}>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '1.05rem', fontWeight: 300, maxWidth: '380px', margin: '0 auto 3rem', lineHeight: 1.8 }}>
