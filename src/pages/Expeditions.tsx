...
function Countdown({ date }: { date: string }) {
  ...
  return (
    <div>
      <p className="mono" style={{ fontSize: '0.68rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: '#888', marginBottom: '1.25rem' }}>
        ...
        {units.map(({ v, l }, i) => (
          <div key={l} style={{ display: 'flex', alignItems: 'flex-start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              <span className="mono" style={{ fontSize: '0.6rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#999', marginTop: '0.5rem' }}>
                {l}
              </span>
            </div>
            {i < units.length - 1 && (
    ...
    <motion.div
      ...
      <motion.div
        ...
        <div style={{ position: 'relative', height: '280px', overflow: 'hidden', flexShrink: 0 }}>
          <img src={exp.image} alt={exp.destination} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
          ...
          <button onClick={onClose}
            style={{ position: 'absolute', top: '1.25rem', right: '1.25rem', width: '2.5rem', height: '2.5rem', background: 'rgba(255,255,255,0.9)', border: 'none', color: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            <X size={16} />
          </button>
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '1.75rem 2.5rem' }}>
            <span className="mono" style={{ fontSize: '0.65rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: '#FF7B00', display: 'block', marginBottom: '0.4rem' }}>
        ...
        <div style={{ padding: '2.5rem 2.5rem 3.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px,1fr))', gap: '0', marginBottom: '2.5rem', border: '1px solid #EBEBEB' }}>
            {[
              ...
              <div key={label} style={{ padding: '1.5rem', borderRight: i < arr.length - 1 ? '1px solid #EBEBEB' : 'none' }}>
                <p className="mono" style={{ fontSize: '0.6rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#999', marginBottom: '0.5rem' }}>{label}</p>
                <p style={{ color: accent ? '#FF7B00' : '#0A0A0A', fontSize: '1rem', fontWeight: 700 }}>{value}</p>
              </div>
            ))}
          </div>

          <p style={{ color: '#555', fontSize: '0.95rem', fontWeight: 300, lineHeight: 1.85, marginBottom: '2.5rem' }}>{exp.description}</p>

          <div style={{ marginBottom: '2.5rem' }}>
            <p className="mono" style={{ fontSize: '0.65rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: '#999', marginBottom: '1.25rem' }}>Destaques</p>
          ...
          <div style={{ marginBottom: '2.5rem' }}>
            <p className="mono" style={{ fontSize: '0.65rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: '#999', marginBottom: '1.25rem' }}>Roteiro</p>
            <div style={{ border: '1px solid #EBEBEB' }}>
              {exp.itinerary.map((day, i) => (
                <div key={i} style={{ display: 'flex', gap: '1.5rem', padding: '1.25rem 1.5rem', borderBottom: i < exp.itinerary.length - 1 ? '1px solid #EBEBEB' : 'none' }}>
                  <span className="mono" style={{ fontSize: '0.6rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#FF7B00', width: '3.5rem', flexShrink: 0, paddingTop: '2px' }}>{day.day}</span>
                  <div>
                    <p style={{ color: '#0A0A0A', fontSize: '0.9rem', fontWeight: 600, marginBottom: '0.25rem' }}>{day.title}</p>
                    <p style={{ color: '#777', fontSize: '0.85rem', fontWeight: 300, lineHeight: 1.7 }}>{day.description}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ background: '#F7F7F5', border: '1px solid #EBEBEB', padding: '1.75rem', marginBottom: '2.5rem' }}>
            <p className="mono" style={{ fontSize: '0.65rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: '#999', marginBottom: '1.25rem' }}>O que está incluso</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.65rem' }}>
              {exp.includes.map((item, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#FF7B00', flexShrink: 0 }} />
                  <span style={{ color: '#444', fontSize: '0.875rem', fontWeight: 400 }}>{item}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', paddingTop: '1.75rem', borderTop: '1px solid #EBEBEB' }}>
            {exp.status === 'proxima' ? (
              ...
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', background: '#F7F7F5', border: '1px solid #EBEBEB' }}>
                <span className="mono" style={{ fontSize: '0.65rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#999' }}>Expedição encerrada</span>
              </div>
            )}
      ...
      <section style={{ background: '#0A0A0A', position: 'relative', overflow: 'hidden', paddingTop: 'clamp(8rem,16vh,12rem)', paddingBottom: 'clamp(4rem,8vh,6rem)' }}>
        ...
        <div className="wrap" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.45 }}
            style={{ color: 'rgba(255,255,255,0.32)', fontSize: '1rem', fontWeight: 300, lineHeight: 1.75, maxWidth: '480px', marginTop: '1.75rem' }}>
          ...
          <div style={{ border: '1px solid #EBEBEB', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative', overflow: 'hidden' }}
              ...
              <div style={{ position: 'absolute', bottom: '2rem', left: '2.5rem' }}>
                <span className="mono" style={{ fontSize: '0.65rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.45)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '2rem', marginBottom: '3rem' }}>
                <div>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.45rem 1rem', background: '#FF7B00', marginBottom: '1.5rem' }}>
                    ...
                    <span className="mono" style={{ fontSize: '0.62rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#fff' }}>
                    ...
                    {[
                      ...
                      <div key={label}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.4rem' }}>
                          ...
                          <span className="mono" style={{ fontSize: '0.6rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#999' }}>{label}</span>
                        </div>
                ...
                <div style={{ textAlign: 'right' }}>
                  <p className="mono" style={{ fontSize: '0.62rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#999', marginBottom: '0.4rem' }}>Investimento</p>
                  <p className="display" style={{ fontSize: 'clamp(2.5rem,4vw,3.5rem)', color: '#0A0A0A' }}>{NEXT.price}</p>
                  <p className="mono" style={{ fontSize: '0.6rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#999', marginTop: '0.3rem' }}>por pessoa</p>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px,1fr))', gap: '0.75rem', marginBottom: '3rem' }}>
                {NEXT.highlights.map((h, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.9rem 1rem', border: '1px solid #EBEBEB' }}>
                    <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#FF7B00', flexShrink: 0 }} />
                    <span style={{ color: '#333', fontSize: '0.875rem', fontWeight: 400 }}>{h}</span>
                  </div>
                ))}
              </div>
              ...
              <Reveal key={exp.id} delay={i * 0.08}>
                ...
                >
                  <div style={{ overflow: 'hidden', aspectRatio: '16/9', position: 'relative' }}>
                    ...
                    <div style={{ position: 'absolute', top: '1rem', left: '1rem', padding: '0.35rem 0.85rem', background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(8px)' }}>
                      <span className="mono" style={{ fontSize: '0.58rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)' }}>
                        Realizada
                      </span>
                    </div>
                  </div>
                  <div style={{ padding: '1.75rem 2rem' }}>
                    <span className="mono" style={{ fontSize: '0.6rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: '#999', display: 'block', marginBottom: '0.35rem' }}>
                      {exp.state} · {exp.dateLabel}
                    </span>
                    <h3 className="display" style={{ fontSize: 'clamp(1.8rem,3.5vw,2.5rem)', color: '#0A0A0A', marginBottom: '0.75rem' }}>
                      {exp.destination.toUpperCase()}
                    </h3>
                    <p style={{ color: '#777', fontSize: '0.875rem', fontWeight: 300, lineHeight: 1.7, marginBottom: '1.25rem' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '1.25rem', borderTop: '1px solid #EBEBEB' }}>
                      <div style={{ display: 'flex', gap: '1.5rem' }}>
                        <span className="mono" style={{ fontSize: '0.58rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#999' }}>{exp.duration}</span>
                        <span className="mono" style={{ fontSize: '0.58rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#999' }}>{exp.spots} participantes</span>
                      </div>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: '#FF7B00', fontSize: '0.75rem', fontWeight: 600 }}>
